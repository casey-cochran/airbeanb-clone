...
const validatePost = [
  check("name")
    ...
    .custom((value) => {
      return Spot.findOne({ where: { name: value } }).then((spotName) => {
        if (spotName) {
  ...
  check("address")
    ...
    .custom((value) => {
      return Spot.findOne({ where: { address: value } }).then((spotAddress) => {
        if (spotAddress) {
  ...
  check("bed")
    .exists({ checkFalsy: true })
    .withMessage("Must provide number of beds"),
  check("room")
    .exists({ checkFalsy: true })
    .withMessage("Must provide number of rooms"),
  handleValidationErrors,
];
