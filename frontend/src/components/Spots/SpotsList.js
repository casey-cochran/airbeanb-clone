        ...
        {newspots?.map((spot, index) => {
          return (
            <div className=".img-description" key={index}>
              ...
              {/*todo spot details <div>hello</div> */}
            </div>
          );
        })}
      </div>
      <div id='spots-list-footer'>
            <div><Link to={{pathname: 'https://github.com/casey-cochran'}} target='_blank' className="footer-text">Github</Link></div>
            <div><Link to={{pathname:'https://linkedin.com/in/casey-cochran-488420219/'}} target='_blank' className="footer-text">Linkedin</Link></div>
      </div>
    </div>
